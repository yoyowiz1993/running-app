...
| `AI_API_KEY` | Optional (for programs, meals) | Google Gemini API key from [Google AI Studio](https://aistudio.google.com/app/apikey). When set, programs and meal suggestions are generated by AI. |
| `AI_RACE_KEY` | Optional (for race search) | Dedicated Gemini API key for race search. When set, race search uses this key and `gemini-2.5-flash` with Google Search grounding. Falls back to `AI_API_KEY` if not set. |
| `AI_MODEL` | Optional | Gemini model for programs/meals; defaults to `gemini-3.1-flash-lite-preview`. Race search always uses `gemini-2.5-flash`. |
